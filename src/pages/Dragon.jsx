    ...
    <div className="container mx-auto space-y-8">
      <div className="px-4 leading-6 mt-6 border-b border-gray-200 w-full py-2 font-semibold text-white hover:bg-gray-900">
        ...
        <a
          ...
          className="flex flex-row items-center gap-x-4 text-sm"
        >
          <span
            className={
              dragon?.active
      ...
      <div className="md:py-8 grid lg:grid-cols-3 md:grid-cols-2">
        ...
        <p className="flex flex-col py-1 px-4">
          first launch
          <span className="text-4xl font-extrabold">
            {new Date(dragon?.first_flight).toLocaleDateString("en-US", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })}
      ...
      <div className="flex flex-row">
        <h1 className="text-2xl text-gray-300 flex items-center pr-4  border-r-2">
          Thrusters
        </h1>
          ...
          {thrusters.map((t) => (
            <div key={t.type} className="grid md:grid-cols-2 border-b-2 py-4">
              ...
              <p className="flex flex-col py-1 px-4">
                thrust
                <span className="text-xl font-bold">
                  {t?.thrust?.kN} N / {t?.thrust?.lbf} lbf{" "}
                </span>
              </p>
            </div>
          ))}
      ...
      <div className="grid md:grid-cols-3 grid-cols-1">
        <p className="flex flex-col py-4 px-2 border-2">
          heat shield
          <span className="text-xl font-bold">
          ...
          <span className="text-xl font-bold">
            <strong>size : </strong>
            {dragon?.heat_shield?.size_meters} meters
          </span>
          <span className="text-xl font-bold">
        ...
        <p className="flex flex-col py-4 px-2 border-2">
          ...
          <span className="text-xl font-bold">
            ...
            {new Intl.NumberFormat("en-US", {
              ...
              unit: "kilogram",
              unitDisplay: "short",
            }).format(dragon?.launch_payload_mass?.kg)}
          </span>
          <span className="text-xl font-bold">
            <strong>launch volume : </strong>
            {new Intl.NumberFormat("en-US", {
              style: "unit",
              unit: "meter",
              unitDisplay: "short",
            }).format(dragon?.launch_payload_vol?.cubic_meters)}
          </span>
          <span className="text-xl font-bold">
            <strong>return mass : </strong>
            {new Intl.NumberFormat("en-US", {
              style: "unit",
              unit: "kilogram",
              unitDisplay: "short",
            }).format(dragon?.return_payload_mass?.kg)}
          </span>
          <span className="text-xl font-bold">
            <strong>return volume : </strong>
            {new Intl.NumberFormat("en-US", {
              style: "unit",
              unit: "meter",
              unitDisplay: "short",
            }).format(dragon?.return_payload_vol?.cubic_meters)}
          </span>
        </p>
        <p className="flex flex-col py-4 px-2 border-2">
          trunk
          <span className="text-xl font-bold">
            <strong>trunk : </strong>
            {dragon?.trunk?.cubic_meters} cbm
          </span>
          <span className="text-xl font-bold">
            <strong>trunk - height : </strong>
            {dragon?.height_w_trunk?.meters} m
          </span>
          <span className="text-xl font-bold">
            <strong>solar array : </strong>
            {dragon?.cargo?.solar_array} m
          </span>
          <span className="text-xl font-bold">
