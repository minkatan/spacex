...
{!dropdown &&
  array.map((r) => (
    ...
    <span className={r.active ? 'text-emerald-500 uppercase' : 'text-red-500 uppercase'}>{r?.active ? 'active' : 'inactive'}</span>
  ...
  <div className="md:py-8 flex flex-col gap-y-2">
    <p className='flex flex-col py-1 px-4'>cost per launch
      <span className="text-3xl font-extrabold">{new Intl.NumberFormat('en-US', {style: 'currency', currency:'USD', notation: "compact",compactDisplay: "long"}).format(r?.cost_per_launch)}</span>
    </p>
    <p className='flex flex-col py-1 px-4'>success %
    <span className="text-3xl font-extrabold">{r?.success_rate_pct}%</span>
    </p>
    <p className='flex flex-col py-1 px-4'>first launch
    <span className="text-3xl font-extrabold">{new Date(r?.first_flight).toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: 'numeric'})}</span>
    </p>
    <p className='flex flex-col py-1 px-4'>height
    <span className="text-3xl font-extrabold">{r?.height?.meters} meters</span>
    </p>
    <p className='flex flex-col py-1 px-4'>diameter
    <span className="text-3xl font-extrabold">{r?.diameter?.meters} meters</span>
    </p>
    <p className='flex flex-col py-1 px-4'>mass
    <span className="text-3xl font-extrabold">{new Intl.NumberFormat('en-US', {style: 'unit', unit:'kilogram', unitDisplay: "short"}).format(r?.mass?.kg)}</span>
    </p>
  </div>
  </>
  ))}
